...
## **Interpreter**
**Interpreter** використовується для інтерпретації користувацьких запитів на трансформацію 3D-моделей за власною мовою (наприклад: `"scale 2 rotate 45"`).

🔹 **Учасники**
- **`IExpression`** *(AbstractExpression)* — визначає інтерфейс `Interpret(Context context)` для всіх виразів.
- **`ScaleExpression`**, **`RotateExpression`** *(TerminalExpression)* — реалізують інтерпретацію термінальних команд, таких як `scale`, `rotate`, `translate`.
- **`SequenceExpression`** *(NonterminalExpression)* — представляє комбінації команд, об'єднані у правила.
...
- **`ModelingMediator`** (Concrete Mediator) – реалізує комунікацію та координує дії між компонентами.
- **`Component`** (Базовий клас) – зберігає посилання на посередника.
- **`MeshEditor`**, **`TextureManager`**, **`GeometryGenerator`** – конкретні компоненти, які спілкуються через посередник.
